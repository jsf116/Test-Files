...
All examples listed below can be found and executed using B<xt/synposis.t>.
...
Verifies the directory B<$got_dir> for the presence of a list files in B<@file_list>. Symlinks are not followed.
...
Compares (not following symlinks!) all files in the directories B<$got_dir> and B<$reference_dir> reporting differences.
